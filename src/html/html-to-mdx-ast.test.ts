...
async function htmlToMdxString({
    ...
    convertAttributeValue?: (args: { name: string, value: string, tagName: string }) => string
}): Promise<string> {
    const markdownProcessor = withProcessor ? unified().use(remarkParse).use(remarkMdx) : undefined
